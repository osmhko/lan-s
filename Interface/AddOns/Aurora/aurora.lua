...
local BDs, SDs, NoBD, NoSD, bordersize = {}, {}, 1, 1, 1
...
Skin:SetScript("OnEvent", function(self, event, addon)
	if addon == "Aurora" then
		...
		-- Colored EquipmentFlyoutFrameButtons
		...
		local EFFUpdate = function(self)
			local buttons, butj = EquipmentFlyoutFrame.buttons ,nil
			for _, button in next, buttons do
				if not button.bg then button.bg = F.CreateBG(button) end
				local popoutitemid = pipe(button)
				if popoutitemid then 
					local quality = select(3, GetItemInfo(popoutitemid))
					if quality and quality > 1 then
						button.bg:SetVertexColor(GetItemQualityColor(quality))
					else
		...
		hooksecurefunc("EquipmentFlyout_Show", EFFUpdate)
	...
	-- elseif addon == "Blizzard_RaidUI" then
		...
		for _, object in pairs(StripAllTextures) do		
			if _G[object] then
				F.StripTextures(_G[object])
				F.CreateBD(_G[object], .2)
			end
		end

		for i=1, MAX_RAID_GROUPS*5 do
			F.StripTextures(_G["RaidGroupButton"..i])
			F.Reskin(_G["RaidGroupButton"..i])
		end
		
		for i=1,8 do
			_G["RaidGroup"..i.."Label"]:Hide()
			for j=1,5 do
				F.StripTextures(_G["RaidGroup"..i.."Slot"..j])
				if select(1,_G["RaidGroup"..i.."Slot"..j]:GetRegions()).UnregisterAllEvents then
					select(1,_G["RaidGroup"..i.."Slot"..j]:GetRegions()):UnregisterAllEvents()
				end
				select(1,_G["RaidGroup"..i.."Slot"..j]:GetRegions()).Show = function() return end
				select(1,_G["RaidGroup"..i.."Slot"..j]:GetRegions()):Hide()
			end
		end
	----------------------------------------------------------------------------
...
Delay:SetScript("OnEvent", function()
	...
	if not(IsAddOnLoaded("MyTooltip") or IsAddOnLoaded("TipTac") or IsAddOnLoaded("FreebTip") or IsAddOnLoaded("lolTip") or IsAddOnLoaded("StarTip") or IsAddOnLoaded("TipTop")) then
		local tooltips = {
			"GameTooltip",
			"ItemRefTooltip",
			"ShoppingTooltip1",
...
local function reloadBD()
	bordersize = 768/string.match(GetCVar("gxResolution"), "%d+x(%d+)")/UIParent:GetEffectiveScale()
	local bc, bdc = {}, {}
	for i = 1, #BDs do
		bc.r, bc.g, bc.b, bc.a = BDs[i]:GetBackdropColor()
		bdc.r, bdc.g, bdc.b, bdc.a = BDs[i]:GetBackdropBorderColor()
		BDs[i]:SetBackdrop({
			bgFile = C.media.backdrop, 
			edgeFile = C.media.backdrop, 
			edgeSize = bordersize, 
		})
		BDs[i]:SetBackdropColor(bc.r, bc.g, bc.b, bc.a)
		BDs[i]:SetBackdropBorderColor(bdc.r, bdc.g, bdc.b, bdc.a)
	end
end
...
local function reloadSD()
	...
	local sbc, sbdc = {}, {}
	for i = 1, #SDs do
		local shadowsize = (mult*math.floor(SDs[i].size/mult+.5))
		local offset = (mult*math.floor((SDs[i].size + 1 + SDs[i].offset)/mult+.5))
		sbdc.r, sbdc.g, sbdc.b = SDs[i]:GetBackdropBorderColor()
		SDs[i]:SetBackdrop({
			edgeFile = C.media.glow, 
			edgeSize = shadowsize, 
		})
		SDs[i]:ClearAllPoints()
		SDs[i]:SetPoint("TOPLEFT", SDs[i]:GetParent(), "TOPLEFT", -offset, offset)
		SDs[i]:SetPoint("BOTTOMRIGHT", SDs[i]:GetParent(), "BOTTOMRIGHT", offset, -offset)
		SDs[i]:SetBackdropBorderColor(sbdc.r, sbdc.g, sbdc.b)
	end
end
