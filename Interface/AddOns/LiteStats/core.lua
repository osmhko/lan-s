...
if latency.enabled then
	Inject("Latency", {
		...
		OnEnter = function(self)
			local _,_,home,world = GetNetStats();
			GameTooltip:SetOwner(self, latency.tip_anchor or "ANCHOR_BOTTOMLEFT", latency.tip_x or 0, latency.tip_y or 0)
			GameTooltip:ClearLines()
			GameTooltip:AddLine(zsub(MAINMENUBAR_LATENCY_LABEL,"\n.*","",":",""), tthead.r, tthead.g, tthead.b)
			GameTooltip:AddDoubleLine(CHANNEL_CATEGORY_WORLD, gradient(1-world/750)..world, ttsubh.r, ttsubh.g, ttsubh.b, 1, 1, 1)
			GameTooltip:AddDoubleLine(HOME, gradient(1-home/750)..home, ttsubh.r, ttsubh.g, ttsubh.b, 1, 1, 1)
			GameTooltip:Show()
		end,
	})
end
------------------------------------------
--  Mail
if mail.enabled then
	MiniMapMailFrame.Show = MiniMapMailFrame.Hide
	Inject("Mail", {
		text = { string = function() return mail[HasNewMail() and "newmail" or "nomail"] end },
		OnEnter = function(self)
			if not HasNewMail() then return end
			GameTooltip:SetOwner(self, mail.tip_anchor, mail.tip_x, mail.tip_y)
...
if memory.enabled then
	...
	Inject("Memory", {
		...
		OnEnter = function(self)
			...
			GameTooltip:ClearLines()
			local _,_,home,world = GetNetStats()
			local lat, r = math.max(home,world), 750
	...
	Inject("Clock", { -- height = 11,
		...
		OnEnter = function(self)
			...
			for i = 1, 2 do
				...
				GameTooltip:AddDoubleLine(format(localizedName,""), isActive and WINTERGRASP_IN_PROGRESS or (startTime==0) and "N/A" or fmttime(startTime),ttsubh.r,ttsubh.g,ttsubh.b,1,1,1)
			end
...
if location.enabled then
	Inject("Location", { -- height = 13, width = 113, 
		OnLoad = function(self)
			MiniMapWorldMapButton:Hide() -- Hide minimap world map button.
			MinimapZoneTextButton:Hide() -- Hide minimap zone text.
...
if dps.enabled then
	...
	Inject("DPS", {
		text = {
			string = function(self)
				self = self:GetParent()
				if dps.hide_inactive and not self.active and self.inactive_time and
				   GetTime() - self.inactive_time > 10 then self:Hide()
				end
				self.dps = self.active and self.damage / (GetTime() - self.starttime) or self.dps or 0
				return format(dps.fmt, short(self.dps))
			end
		},
		OnLoad = function(self)
			RegEvents(self,"UNIT_PET COMBAT_LOG_EVENT_UNFILTERED PLAYER_REGEN_ENABLED PLAYER_REGEN_DISABLED")
			self.events = "SWING_DAMAGE RANGE_DAMAGE SPELL_DAMAGE SPELL_PERIODIC_DAMAGE DAMAGE_SHIELD DAMAGE_SPLIT"
			self.inactive_time = GetTime()
			self.player, self.pet, self.damage, self.active = UnitGUID(P), UnitGUID'pet' or '0x0', 0, false
			if dps.hide_inactive then self:Hide() end
		end,
		OnEvent = function(self, event, ...)
			if event == "COMBAT_LOG_EVENT_UNFILTERED" then
				local params = {...}
				local spellevent = params[2]
				local unit = params[4]
				local damage = params[spellevent == "SWING_DAMAGE" and 12 or 15]
				...
				if unit == self.player or unit == self.pet then
					if self.damage == 0 then
						...
						self.text:SetText(format(dps.fmt, 0))
					end
					if not self:IsShown() and dps.hide_inactive then self:Show() end
		...
		OnClick = function(self, button)
			if button == "RightButton" then
				...
				self.text:SetText(format(dps.fmt, short(self.dps)))
			end
		end
	})
end
...
if friends.enabled then
	Inject("Friends", {
		...
		OnEnter = function(self)
			...
			local numBNetTotal, numBNetOnline = BNGetNumFriends()
			local online, total = 0, GetNumFriends()
			local name, level, class, zone, connected, status, note, zone_r, zone_g, zone_b, grouped
			for i = 0, total do if select(5, GetFriendInfo(i)) then online = online + 1 end end
			online=online+numBNetOnline
			total=total+numBNetTotal
			if online > 0 then
				GameTooltip:SetOwner(self, friends.tip_anchor, friends.tip_x, friends.tip_y)
				GameTooltip:ClearLines()
				GameTooltip:AddDoubleLine(FRIENDS_LIST, format("%s: %s/%s",GUILD_ONLINE_LABEL,online,total),tthead.r,tthead.g,tthead.b,tthead.r,tthead.g,tthead.b)
				GameTooltip:AddLine' '
			
				-- track redundancy of battle/wow friends
				local fList = {}
			
				for i =1,numBNetOnline do
					presenceID, givenName, surname, toonName, toonID, client, connected, lastOnline, isAFK, isDND, broadcastText, noteText, isFriend, broadcastTime = BNGetFriendInfo(i)
					local numToons = BNGetNumFriendToons(i)
					for m=1,numToons do
						local _, name, client, realm, _, _, race, class, _, zone, level = BNGetFriendToonInfo(i,m)
						if not connected then break end
						tinsert(fList, name)
					
						local status = isAFK and FRIENDS_TEXTURE_AFK or isDND and FRIENDS_TEXTURE_DND or FRIENDS_TEXTURE_ONLINE
					
						for k,v in pairs(LOCALIZED_CLASS_NAMES_MALE) do if class == v then class = k end end
						if GetLocale() ~= "enUS" then -- feminine class localization (unsure if it's really needed)
							for k,v in pairs(LOCALIZED_CLASS_NAMES_FEMALE) do if class == v then class = k end end
						end
						local classc, levelc = (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[class] or {r=1,g=1,b=1}, GetQuestDifficultyColor(level ~= "" and level or 1)
						if GetRealZoneText() == zone then zone_r, zone_g, zone_b = 0.3, 1.0, 0.3 else zone_r, zone_g, zone_b = 0.65, 0.65, 0.65 end
						grouped = client ~= "WoW" and "" or (UnitInParty(name) or UnitInRaid(name)) and (GetRealZoneText() == zone and " |cff7fff00*|r" or " |cffff7f00*|r") or ""
						zone = client ~= "WoW" and format("|T%s:%d|t",client ~= "WoW" and "Interface\\FriendsFrame\\Battlenet-Sc2icon",24) or zone
						name = client == "WoW" and " "..name or format("|cff67D6FF%s %s|r%s",givenName, surname, (name and " |cffaaaaaa("..name..")|r" or "")) 
						GameTooltip:AddDoubleLine(format("|T%s:%d|t|cff%02x%02x%02x%s|r|cff%02x%02x%02x%s|r%s",status,20,levelc.r*255,levelc.g*255,levelc.b*255,level,classc.r*255,classc.g*255,classc.b*255,name,grouped),zone,1,1,1,zone_r,zone_g,zone_b)
						if self.altdown then GameTooltip:AddLine(" "..(client == "WoW" and format("|cffffffff%s %s%s |r",givenName,surname,((noteText and noteText ~= "") and ": "..noteText or "")) or ""),ttsubh.r,ttsubh.g,ttsubh.b,1) end
					end
				end
				...
				for i = 1, GetNumFriends() do
					...
					if not connected then break end
					if not tContains(fList, name) then
						status = string.find(status,AFK) and FRIENDS_TEXTURE_AFK or string.find(status,AFK) and FRIENDS_TEXTURE_DND or FRIENDS_TEXTURE_ONLINE
						if GetRealZoneText() == zone then zone_r, zone_g, zone_b = 0.3, 1.0, 0.3 else zone_r, zone_g, zone_b = 0.65, 0.65, 0.65 end
						for k,v in pairs(LOCALIZED_CLASS_NAMES_MALE) do if class == v then class = k end end
						if GetLocale() ~= "enUS" then -- feminine class localization (unsure if it's really needed)
							for k,v in pairs(LOCALIZED_CLASS_NAMES_FEMALE) do if class == v then class = k end end
						...
						grouped = (UnitInParty(name) or UnitInRaid(name)) and (GetRealZoneText() == zone and " |cff7fff00*|r" or " |cffff7f00*|r") or ""
						GameTooltip:AddDoubleLine(format("|T%s:%d|t|cff%02x%02x%02x%d|r %s%s",status,20,levelc.r*255,levelc.g*255,levelc.b*255,level or "",name,grouped),zone,classc.r,classc.g,classc.b,zone_r,zone_g,zone_b)
					...
					for i = 1, GetNumTalentGroups() do
						...
						for tree = 1, GetNumTalentTabs() do
							...
							if tal[tree][5] ~= 0 and tal[tree][5] > pts then
								name, icon, pts = {tal[tree][2],tree}, tal[tree][4], tal[tree][5]
							end
						end
