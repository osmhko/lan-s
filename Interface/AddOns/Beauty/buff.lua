...
buffholder:Point("TOPRIGHT", UIParent, "TOPRIGHT",  -185, -7)
...
debuffholder:Point("TOPRIGHT", UIParent, "TOPRIGHT", -185, -145)
...
enchantholder:Point("TOPLEFT", Minimap, "BOTTOMLEFT", 0, -43)
...
local function StyleBuffs(button, index, framekind, anchor)
    ...
    if framekind == 3 then
    	_G[buff.."Count"]:SetFont(C.font, 10, "OUTLINE")
    	_G[buff.."Duration"]:SetFont(C.font, 10, "OUTLINE")
    	_G[buff]:Height(buttonsize - 8)
		_G[buff]:Width(buttonsize - 8)
    else
		_G[buff.."Count"]:SetFont(C.font, 14, "OUTLINE")
   		_G[buff.."Duration"]:SetFont(C.font, 14, "OUTLINE")
