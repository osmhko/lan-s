...
elseif ct.myclass=="PRIEST"then
	if(ct.mergeaoespam)then
		...
		ct.aoespam[15237]=true		-- Holy Nova (Damage Effect)
		ct.aoespam[88686]=true		-- 圣言术：庇
		
	end
	if(ct.healing)then
		ct.healfilter[2944]=true 	-- Devouring Plague (Healing)
		ct.healfilter[15290]=true	-- Vampiric Embrace
		ct.healfilter[77489]=true	-- Echo of light
		ct.healfilter[88686]=false	-- 圣言术：庇
	end
...
if(event=="COMBAT_TEXT_UPDATE")then
	...
	elseif subevent=="DAMAGE_CRIT"then
		xCT1:AddMessage(ct.incomecritprefix.."-"..arg2..ct.incomecritpostfix,1,.1,.1)
	elseif subevent=="	"then
		xCT1:AddMessage("-"..arg2,.75,.3,.85)
	elseif subevent=="SPELL_DAMAGE_CRIT"then
		xCT1:AddMessage(ct.incomecritprefix.."-"..arg2..ct.incomecritpostfix,1,.3,.5)
...
for i=1,numf do
	...
	if(i==1)then
		f:SetJustifyH(ct.justify_1)                         --受到的伤害
		f:SetPoint("BOTTOMLEFT",UIParent, "BOTTOM", -310, 510)
		f:SetHeight(190)
		f:SetWidth(128)
	...
	elseif(i==2)then
		f:SetJustifyH(ct.justify_2)                   --受到的治疗
	...
	elseif(i==3)then
		f:SetJustifyH(ct.justify_3)                    --战斗信息
		f:ClearAllPoints()
		f:SetPoint("CENTER",0,192)
		f:SetHeight(128)
		f:SetWidth(256)
	else
		f:SetJustifyH(ct.justify_4)                  --输出伤害/治疗    
		f:SetPoint("BOTTOMRIGHT",UIParent, "BOTTOM",  410, 470)---280, 420
		f:SetHeight(260)
		f:SetWidth(192)
		local a,_,c=f:GetFont()
		if (ct.damagefontsize=="auto")then
			if ct.icons then
				f:SetFont(a,ct.iconsize*2/3,c)
			end
		elseif (type(ct.damagefontsize)=="number")then
			f:SetFont(a,ct.damagefontsize,c)
		end
		if(i==5)then
			f:ClearAllPoints()
			f:SetPoint("BOTTOMLEFT",UIParent, "BOTTOM", 405, 466)
			f:SetJustifyH(ct.justify_5)
			f:SetFont(a,ct.fontsize*2/3+ct.critfix+8,c)
			f:SetWidth(192)
		end
	end
	ct.frames[i] = f
	...
	ct.frames[i]:SetScript("OnUpdate",function(self,elapsed)
		...
		if(TimeSinceLastUpdate>UpdateInterval)then
			...
			ct.frames[i]:AddMessage(COMBAT_TEXT_LABEL,random(255)/255,random(255)/255,random(255)/255)
				...
				if(icon)then
					if (i==5) then
						msg=ct.critprefix.." \124T"..icon..":"..ct.iconsize..":"..ct.iconsize..":0:0:64:64:5:59:5:59\124t"..msg..ct.critpostfix
						else msg=msg.." \124T"..icon..":"..ct.iconsize..":"..ct.iconsize..":0:0:64:64:5:59:5:59\124t"
					end
					if(ct.damagecolor)then
...
SlashCmdList["XCT"]=function(input)
	...
	elseif(input=="mypos")then
		xCT1:ClearAllPoints()
		xCT1:SetPoint("BOTTOMLEFT",UIParent, "BOTTOM", -310, 420)
		xCT1:SetHeight(260)
		xCT1:SetWidth(128)
		xCT2:ClearAllPoints()
		xCT2:SetPoint("BOTTOMLEFT",UIParent, "BOTTOM", -200, 420)
		xCT2:SetHeight(260)
		xCT2:SetWidth(128)
		xCT3:ClearAllPoints()
		...
		xCT3:SetWidth(256)
		xCT4:ClearAllPoints()
		xCT4:SetPoint("TOPLEFT",UIParent, "BOTTOM", 310, 420)
		xCT4:SetHeight(260)
		xCT4:SetWidth(128)
		xCT5:ClearAllPoints()
		xCT5:SetPoint("BOTTOMRIGHT",UIParent, "BOTTOM", 390, 420)
		xCT5:SetHeight(260)
		xCT5:SetWidth(128)
	else
...
if(ct.mergeaoespam)then
	if (ct.damage or ct.healing) then
		if (not ct.mergeaoespamtime or ct.mergeaoespamtime<0.1) then
			ct.mergeaoespamtime=0.1
		end
		local pairs=pairs
		SQ={}
				...
				for k,v in pairs(SQ) do
					if not SQ[k]["locked"] and SQ[k]["queue"]>0 and SQ[k]["utime"]+ct.mergeaoespamtime<=utime then
						if SQ[k]["count"]>1 then
							count=" |cffFFFFFF x|r"..SQ[k]["count"].."|r"
						else
							count=""
						end
						xCT4:AddMessage(count..SQ[k]["msg"]..SQ[k]["queue"], unpack(SQ[k]["color"]))
						SQ[k]["queue"]=0
						SQ[k]["count"]=0
					end
				end
		...
		if(sourceGUID==ct.pguid and destGUID~=ct.pguid)or(sourceGUID==UnitGUID"pet" and ct.petdamage)or(sourceFlags==gflags)then
			if(eventType=="SWING_DAMAGE")then
				...
				if(amount>=ct.treshold)then
					...
					if(ct.icons)then
						...
						if (critical) then
							msg=" \124T"..icon..":"..ct.iconsize..":"..ct.iconsize..":0:0:64:64:5:59:5:59\124t"..msg
						else msg=msg.." \124T"..icon..":"..ct.iconsize..":"..ct.iconsize..":0:0:64:64:5:59:5:59\124t"
						end
					end
			...
			elseif(eventType=="SPELL_DAMAGE")or(eventType=="SPELL_PERIODIC_DAMAGE" and ct.dotdamage)then
				...
				if(amount>=ct.treshold)then
					...
					if (critical) then xCT5:AddMessage(msg..""..amount,unpack(color))	else xCT4:AddMessage(amount..""..msg,unpack(color)) end
				end
			...
			if(eventType=='SPELL_HEAL')or(eventType=='SPELL_PERIODIC_HEAL'and ct.showhots)then
				if(ct.healing)then
					...
					if(amount>=ct.healtreshold)then
						...
						if (critical) then xCT5:AddMessage(msg..""..amount,unpack(color)) else
