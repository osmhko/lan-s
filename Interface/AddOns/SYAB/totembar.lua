...
local function StyleTotemFlyout(flyout)
	...
	for _,button in ipairs(flyout.buttons) do
		...
		button:StyleButton("setallpoints")
	end
	...
	close:Height(8)
--	close:Hide()
--	close:HookScript("OnShow", function(self) self:Hide() end)
...
local function StyleTotemOpenButton(button, parent)
	...
	if not button.visibleBut then
		...
		button.visibleBut:Width(C["actionbar"].stancebuttonsize)
		...
		button.visibleBut.highlight:Point("TOPLEFT",button.visibleBut,"TOPLEFT",-2,2)
		button.visibleBut.highlight:Point("BOTTOMRIGHT",button.visibleBut,"BOTTOMRIGHT",2,-2)
...
local function StyleTotemSlotButton(button, index)
	...
	button.background:Point("TOPLEFT",button,"TOPLEFT",0, -0)
	button.background:Point("BOTTOMRIGHT",button,"BOTTOMRIGHT",-0, 0)
	-- button.background:SetAllPoints()
	if not InCombatLockdown() then button:Size(C["actionbar"].stancebuttonsize) end
...
local function StyleTotemActionButton(button, index)
	...
	icon:SetTexCoord(.09,.91,.09,.91)
	icon:SetDrawLayer("ARTWORK")
	icon:Point("TOPLEFT",button,"TOPLEFT",0,0)
	icon:Point("BOTTOMRIGHT",button,"BOTTOMRIGHT",0,0)
	button.overlayTex:SetTexture(nil)
	...
	button:StyleButton("setallpoints")
	_G[button:GetName().."HotKey"]:ClearAllPoints()
	_G[button:GetName().."HotKey"]:SetPoint("TOPRIGHT", 0, 0)
	_G[button:GetName().."HotKey"]:SetFont(C.font, B.mult*10, "OUTLINE")
end
