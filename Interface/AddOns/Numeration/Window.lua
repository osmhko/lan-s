...
window.bd = CreateFrame("Frame", nil, window)
window.bd:SetFrameLevel(1)
window.bd:SetFrameStrata(window:GetFrameStrata())
window.bd:SetPoint("TOPLEFT",-4,4)
window.bd:SetPoint("BOTTOMRIGHT",4,-4)
window.bd:SetBackdrop({
	bgFile = "Interface\\Buttons\\WHITE8x8", 
	edgeFile = "Interface\\Addons\\media\\glow", 
		tile = false, tileSize = 4, edgeSize = 4, 
		insets = { left = 4, right = 4, top = 4, bottom = 4 }
})
window.bd:SetBackdropBorderColor(0,0,0,1)
window.bd:SetBackdropColor(0,0,0,0.5)
addon.window = window
...
local menuTable = {
	{ text = "数据统计", isTitle = true, notCheckable = true, notClickable = true },
...
function window:ShowResetWindow()
	if not reset then
		...
		reset.titletext:SetPoint("TOP", 0, -1)
		...
		reset.yes:SetPoint("BOTTOMLEFT", 10, 8)
		...
		reset.no:SetPoint("BOTTOMRIGHT", -10, 8)
		...
		local tex = reset.no:CreateTexture(nil, "BACKGROUND")
		tex:SetPoint("TOPLEFT")
		tex:SetPoint("BOTTOMRIGHT")
		tex:SetTexture("Interface\\ChatFrame\\ChatFrameBackground")
		tex:SetGradientAlpha("VERTICAL", 0, 0, 0, .3, .35, .35, .35, .35)
...
function window:ShowWhisperWindow()
	if not whisper then
		...
		whisper.target:SetPoint("BOTTOMLEFT", 10, 8)
		...
		whisper.yes:SetPoint("BOTTOMRIGHT", -10, 8)
