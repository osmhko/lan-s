...
local  function UpdateHealth(self, event, unit)
	...
	if not UnitIsEnemy("player",unit) then
		...
		if UnitIsDeadOrGhost (unit) or disconnected then
			...
			--hp.bg:SetGradientAlpha("VERTICAL",D.r*.2, D.g*.2, D.b*.2,.6,.25,.25,.25,.6)
			return
		end
		...
		if  UnitPlayerControlled(unit) and not UnitIsPlayer(unit) then
			...
			--hp.bg:SetGradientAlpha("VERTICAL",V.r*.2, V.g*.2, V.b*.2,.25,.25,.25,.6)
			hp.bg:SetVertexColor(V.r*.2, V.g*.2, V.b*.2)
			return
		elseif ns.db.definecolors then
			if ns.db.classbgcolor then
				--hp.bg:SetGradientAlpha("VERTICAL",r, g, b,.15,.25,.25,.25,.6)
				hp.bg:SetVertexColor(r, g, b)
			else
				--hp.bg:SetGradientAlpha("VERTICAL",Hbg.r, Hbg.g, Hbg.b,.15,.25,.25,.25,.6)
			...
			if not hp.colorSmooth then
				...
				hp:SetStatusBarColor(ColorGradient(perc))
			end
			return 
		elseif ns.db.reversecolors  then
			--hp.bg:SetGradientAlpha("VERTICAL",r*.2, g*.2, b*.2,.15,.25,.25,.25,.6)
		...
		--hp.bg:SetGradientAlpha("VERTICAL",E.r*.2, E.g*.2, E.b*.2,.15,.25,.25,.25,.6)
...
local style = function(self)
    ...
    Health.bg = Health:CreateTexture(Media.statusBar, "BORDER")
    Health.bg:SetAllPoints(Health)
	Health.bg:SetGradientAlpha("VERTICAL",.2,.2,.2,.15,.25,.25,.25,.6)
    Health.Override = UpdateHealth
	self.Health = Health
    ns:UpdateHealth(self.Health)
	
	self.gradient = self.Health:CreateTexture(nil, "OVERLAY")
	self.gradient:SetPoint("TOPLEFT", Health, 0, -0)
	self.gradient:SetPoint("BOTTOMRIGHT", Health, -0, 0)
	self.gradient:SetTexture(Media.statusBar)
	self.gradient:SetGradientAlpha("VERTICAL",.2,.2,.2,.15,.25,.25,.25,.6)
    ...
    hl:SetTexture([=[Interface\AddOns\oUF_Freebgrid\media\blank.tga]=])
    ...
    if ns.db.multi then
        ...
        for i=1, 8 do
            ...
            if i == 1 then
				...
				--group:SetPoint("BOTTOM", UIParent, "BOTTOM", 0, 130)
            else
