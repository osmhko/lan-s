...
local function CheckRole(self, event, unit)
	...
	if (B.myclass == "PALADIN" and tree == 1) or (B.myclass == "SHAMAN" and tree == 3) or (B.myclass == "PRIEST" and (tree == 1 or tree == 2)) or (B.myclass == "DRUID" and tree == 3) then
		NP.isHealer = true
	else
		NP.isHealer = false
	end
...
local function CreateVirtualFrame(parent, point)
	...
	parent.backdrop:SetBackdrop({
		bgFile = C.blank,
		edgeFile = C.glow,
		edgeSize = 4*noscalemult,
		insets = {
			top = 4*noscalemult, left = 4*noscalemult, bottom = 4*noscalemult, right = 4*noscalemult
		}
	})
	parent.backdrop:SetPoint('TOPLEFT', point, -4*noscalemult, 4*noscalemult)
	parent.backdrop:SetPoint('BOTTOMRIGHT', point, 4*noscalemult, -4*noscalemult)
...
local function CreateAuraIcon(parent)
	...
	button.shadow:SetBackdrop( { 
		...
		bgFile = C.blank,
		edgeSize = B.Scale(4),
		insets = {left = B.Scale(4), right = B.Scale(4), top = B.Scale(4), bottom = B.Scale(4)},
	})
	...
	button.bg2:SetTexture(unpack(C.backdropcolor))
	...
	button.text:SetFont(C.font, 10, "OUTLINE")
	button.text:SetShadowColor(0, 0, 0, 0)

	button.count = button:CreateFontString(nil,"OVERLAY")
	button.count:SetFont(C.font,9,C.fontflag)
...
local function Colorize(frame)
	...
	for class, color in pairs(RAID_CLASS_COLORS) do
		...
		if RAID_CLASS_COLORS[class].r == r and RAID_CLASS_COLORS[class].g == g and RAID_CLASS_COLORS[class].b == b then
			...
			frame.hp:SetStatusBarColor(unpack(NP.colors.class[class]))
			return
		end
	end
	
	if g+b == 0 then -- hostile
		r,g,b = unpack(NP.colors.reaction[1])
		frame.isFriendly = false
	elseif r+b == 0 then -- friendly npc
		r,g,b = unpack(NP.colors.power["MANA"])
		frame.isFriendly = true
	elseif r+g > 1.95 then -- neutral
		r,g,b = unpack(NP.colors.reaction[4])
		frame.isFriendly = false
	elseif r+g == 0 then -- friendly player
		r,g,b = unpack(NP.colors.reaction[5])
		frame.isFriendly = true
	else -- enemy player
		frame.isFriendly = false
	end
...
local function UpdateObjects(frame)
	...
	if frame.hp.boss:IsShown() then
		frame.hp.name:SetText(NP.RGBToHex(0.8, 0.05, 0).." ??|r "..frame.hp.oldname:GetText())
	else
		frame.hp.name:SetText(NP.RGBToHex(frame.hp.oldlevel:GetTextColor())..level..(elite and "+ |r" or "|r ")..frame.hp.oldname:GetText())
	end	
...
local function SkinObjects(frame)
	...
	hp:SetStatusBarTexture(C.normal)
	...
	hp.value:SetFont(C.font, FONTSIZE, C.fontflag)
	...
	hp.value:SetShadowOffset(B.mult, -B.mult)
	...
	hp.name:SetFont(C.font, FONTSIZE, C.fontflag)
	hp.name:SetJustifyH("LEFT")
	hp.name:SetShadowColor(0, 0, 0, 0.4)
	hp.name:SetShadowOffset(B.mult, -B.mult)
	hp.oldname = oldname
	...
	raidicon:SetTexture([[Interface\AddOns\media\raidicons.blp]])	
...
local function ShowHealth(frame, ...)
	...
	if frame.hasClass == true or frame.isFriendly == true then
		if(d <= 50 and d >= 20) then
			frame.hp.backdrop:SetBackdropBorderColor(1, 1, 0, 1)--黄
		elseif(d < 20) then
			frame.hp.backdrop:SetBackdropBorderColor(1, 0, 0, 1)--红
		else
			frame.hp.backdrop:SetBackdropBorderColor(0, 0, 0, 1)--黑
		end
